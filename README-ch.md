...
实验性 / pre-alpha。Consumer Bridge、Storage、`wx.login` 客户端认证启动、`wx.request` HTTP transport、App lifecycle、部分微信页面栈导航、运行时能力检测、权限生命周期、微信会话有效性检查、微信剪贴板与震动、微信基础文件系统访问，以及按需单次微信定位已经实现并通过规定的真实宿主验证。隐私授权已实现但仍取决于小程序的后台隐私配置与账号，真实宿主验收尚未完成。位置权限宿主运行覆盖了 `NotRequested`、`Granted` 与 `Denied`。微信扫码已实现并具备自动化覆盖，其真实宿主验收尚未完成。其他能力的准确状态见 [微信能力矩阵](docs/platforms/wechat/WECHAT_CAPABILITIES-ch.md)。
