...
- WeChat page-stack and tab navigation bridge implemented; `navigateTo`, `redirectTo`, `navigateBack`, and `switchTab` have the required host evidence
...
The WeChat navigation bridge adapts `wx.navigateTo`, `wx.redirectTo`, `wx.navigateBack`, and `wx.switchTab`, and maps a bounded stack, an unknown route, going back from the first page, and a `switchTab` route with no tab to `MiniAppException.HostFailure` rather than reporting success. `switchTab` carries WeChat's own constraint: the route must be declared in the mini program's `tabBar`, the SDK keeps no list of tabBar pages to check against, and a blank route is refused before the host is called. Automated checks pass. The three page-stack operations passed Developer Tools acceptance on 2026-09-15; on 2026-09-18 Developer Tools and an Android device verified a successful tab switch, `SHOWN`/`HIDDEN` without an erroneous `UNLOADED`, and a closed `HostFailure` for a non-tab route.
...
- WeChat Developer Tools loads the artifact, logs the SDK version, and displays the same value on the index page. The recorded run logged `0.1.0-SNAPSHOT`, the version the catalog held then; this tree reports `0.1.0`.
...
- `WechatNavigation` adapts `wx.navigateTo`, `wx.redirectTo`, `wx.navigateBack`, and `wx.switchTab`, is reachable only through `WechatPlatformApi`, and reports WeChat's failure callback as `MiniAppException.HostFailure`.
- The JavaScript and TypeScript facade exposes the App and Page lifecycle entry points, the lifecycle state and page route, and the four navigation functions.
...
- WeChat Developer Tools verifies the foreground lifecycle state, page route, the complete `navigateTo` → `redirectTo` → `navigateBack` page-stack path, and both accepted and refused `switchTab` paths. An Android device additionally verifies `switchTab` with tab-page `SHOWN`/`HIDDEN` semantics.
...
- BLE adapter and discovery were exercised on 2026-09-18 on OnePlus PLQ110 / Android 36 / WeChat 8.0.76: the three capability keys read `Supported`, adapter open and close succeeded, discovery started and stopped, 32 devices were reported, and the listener count fell 2 → 1 → 0. Connection and disconnection were not run, because no connectable peripheral was available, so this is a partial real-device record rather than a complete BLE acceptance. Automated adapter, interop and session tests pass, and the CommonJS smoke drives the whole surface against a fake host, but none of them substitutes for a host run.
...
| `./gradlew --no-daemon --console=plain verifyMiniAppConsumerDocs` | VERIFIED — both READMEs document 21 sections and the same 12 code blocks in the same order, and every `fixtures/miniapp-consumer/...` path they cite exists |
| `./gradlew --no-daemon --console=plain verifyMiniAppBundleSize` | VERIFIED — the SDK's production distribution measures 1,538,258 bytes raw and 279,386 bytes gzip across 12 files, and a second run over the same files produces the same report |
...
The same consumer-facing artifact was verified in WeChat Developer Tools. The console and rendered page both show the SDK version; the recorded run shows `0.1.0-SNAPSHOT`, which is the value the catalog held at that time and is kept as the historical evidence it is. The Consumer Bridge milestone is complete.
