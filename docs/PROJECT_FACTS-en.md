...
`:sdk` is the Kotlin/JS runtime SDK described by the rest of this document. `:miniapp-gradle-plugin` is the `io.github.bobcgn.miniapp` Gradle plugin skeleton. It owns build integration and developer experience only: it detects the Kotlin Multiplatform plugin, registers the `miniapp` platform target chosen in [ADR 0010](decisions/0010-miniapp-gradle-plugin-source-set-model-en.md), and fails with a named error when the Kotlin Multiplatform plugin is absent. It provisions no source-set hierarchy, wires no SDK dependency, assembles no WeChat artifact, exposes no Mini App DSL, and contains no WeChat runtime code.
