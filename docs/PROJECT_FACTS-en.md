...
- npm publication
- Maven publication
- A Mini App Gradle DSL; host business configuration is BOB-84's scope. The plugin assembles a compiler-managed Mini App distribution but performs no upload, publication or host deployment, and no mini app host has loaded it: the distribution's host compatibility is unvalidated
...
| `./gradlew --no-daemon --console=plain :miniapp-gradle-plugin:test` | VERIFIED — 13 tests, no failures |
