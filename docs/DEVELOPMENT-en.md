...
The lifecycle and navigation bridges completed their required host acceptance. The 2026-09-15 Developer Tools run covered the foreground lifecycle state, the current page route, and the complete page-stack path: `navigateTo` opened the second page, `redirectTo` replaced it with the third page, and `navigateBack` returned directly to the index page. On 2026-09-18, Developer Tools and an Android device covered `switchTab`: the tab target reported `SHOWN`, switching away reported `HIDDEN` rather than `UNLOADED`, and a route with no tab produced only the closed `HostFailure` category. The background-state transition still requires placing the mini program in the background on a real device and remains a limitation of the app-level lifecycle evidence, not of the completed navigation acceptance.
