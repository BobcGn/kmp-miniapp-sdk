...
同一规则也经 Gradle 插件作用于消费者。`checkMiniAppHostBoundary` 会让 runtime classpath 携带 Compose、Skiko、Compose 专用的 AndroidX 集成 artifact、`kotlinx-browser` 或 `kotlinx-html` 的 Mini App 构建失败，并且在宿主 distribution 组装之前运行。非 Compose 的 lifecycle、saved-state 与 navigation primitives 仍属于合法的共享行为，不会被拒绝。没有它，一个 common source set 中含 Compose UI 的消费者会构建出任何宿主都无法运行的小程序产物 —— 构建成功，产物却不可用。它是依赖规则而不是文件规则，因为 Kotlin/JS distribution 中的文件通过 `require()` 彼此引用。
