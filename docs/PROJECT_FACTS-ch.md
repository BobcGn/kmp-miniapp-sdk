...
| `./gradlew --no-daemon --console=plain verifyMiniAppConsumerDocs` | VERIFIED — 两份 README 均记录 21 个章节与顺序一致的 12 个代码块，且其引用的每个 `fixtures/miniapp-consumer/...` 路径都存在 |
| `./gradlew --no-daemon --console=plain verifyMiniAppBundleSize` | VERIFIED — SDK 生产分发为 12 个文件、1,538,258 原始字节、279,386 gzip 字节，且对同一批文件再次运行产生完全相同的报告 |
