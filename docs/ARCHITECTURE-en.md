...
The `:miniapp-gradle-plugin` module is this plugin. It registers the `miniapp` target together with that target's Node.js test run, adds the runtime SDK to `miniappMain`, and rejects a project that does not apply the Kotlin Multiplatform plugin. Those registrations are the whole of its model provisioning: `miniappMain`, `miniappTest`, the compilations that own them and their `commonMain` / `commonTest` edges all come from the Kotlin Gradle Plugin, and the plugin never hand-builds a source set or a hierarchy edge, because hand-built ones are the unused-source-set model [ADR 0010](decisions/0010-miniapp-gradle-plugin-source-set-model-en.md) rejected. Separately, it materializes the conventional `src/miniappMain/kotlin` and `src/miniappTest/kotlin` directories during plugin application so an IDE import exposes both empty source sets regardless of the project wizard used. This filesystem convenience is idempotent, preserves existing sources, and does not change ownership of the source-set model.
...
The host artifact is a compiler-managed distribution: `assembleMiniAppBundle` republishes the Mini App target's Kotlin/JS production library at a stable consumer path, so a host integration depends on a documented directory rather than on a Kotlin Gradle Plugin build directory. It contains the consumer's compiled module and its TypeScript declaration, the runtime SDK as its own module, and the Kotlin runtime the compilation resolved, and no markup. Task success alone only establishes a stable host-integration input; host compatibility needs separate evidence. The consumer fixture supplies that evidence: its assembled distribution loaded in WeChat Developer Tools at base library 3.17.3 and reported `fixture.result=PASS`.

Because a distribution that a host cannot run would still build successfully, the plugin rejects the dependency graph that produces one. `checkMiniAppHostBoundary` fails when the Mini App runtime classpath carries a client renderer — Compose, Skiko, a Compose-specific AndroidX integration artifact, `kotlinx-browser` or `kotlinx-html` — and it runs before assembly, so an unusable bundle is never produced. Non-Compose lifecycle, saved-state and navigation primitives remain valid shared behaviour and are not rejected. A consumer's `miniappMain` may inherit shared behaviour and state from the common source set, but not Compose UI: Compose UI belongs in a client module or source set that is not a parent of `miniappMain`. Enforcement is a dependency rule, not a file rule, because the files in a Kotlin/JS distribution reach one another through `require()` and deleting the ones that look like a renderer would trade a build failure for a load failure. A classpath that did not fully resolve is refused rather than read, because Gradle's resolution result lists only what resolved: reading it alone would report "no renderer" for a graph the check never saw.

`fixtures/miniapp-consumer` is the repository's own consumer: an ordinary build beside this one that applies the plugin by id and resolves the runtime by its public coordinate, with no knowledge of this repository's structure. It is the executable form of this boundary — the SDK is consumed, the host integration is packaged, and the host itself is UI. Its WeChat host is WXML, WXSS and thin JavaScript, and that markup stays in the host: it never enters the SDK or the consumer's Kotlin, because a host's view vocabulary is not a platform's.

The consumer-facing build configuration is one extension, `miniapp`, whose only host is `wechat`. The hierarchy is the point: `MiniAppExtension` is the platform and `WeChatHostConfiguration` is a host, so another host is another subtype and another accessor rather than a widened WeChat type. It carries only the host's bundle directory — every field in a Gradle DSL is a decision a consumer must make, and the output shape, CommonJS, declarations and the renderer boundary are the plugin's decisions, not the consumer's. Business and backend configuration is not modelled here for the same reason it is not modelled at runtime.
