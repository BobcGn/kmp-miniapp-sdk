...
Stage E's implementation is in place and awaits external consumer acceptance: `assembleMiniAppBundle` republishes the Mini App target's Kotlin/JS production library — the consumer's module and declaration plus the runtime modules it resolved — at `build/miniapp/bundle/`, and the consumer writes no Kotlin/JS build configuration. The distribution is a stable host-integration input, not a validated artifact: no mini app host has loaded one. `checkMiniAppHostBoundary` rejects a runtime classpath carrying a client renderer before assembly, because such a distribution builds successfully and is still unusable, and the Mini App source set must therefore not inherit Compose UI from a common source set. Stage F's implementation is in place: the plugin registers a `miniapp` extension whose only host is `wechat`, carrying one build setting — the host bundle directory — with the host modelled as a `MiniAppHostConfiguration` subtype rather than as the platform. Business and backend configuration stays out of Gradle by design.
