...
Stage E is accepted: `assembleMiniAppBundle` republishes the Mini App target's Kotlin/JS production library — the consumer's module and declaration plus the runtime modules it resolved — at `build/miniapp/bundle/`, and the consumer writes no Kotlin/JS build configuration. Stage G subsequently proved that distribution in WeChat Developer Tools at base library 3.17.3. `checkMiniAppHostBoundary` rejects a runtime classpath carrying a client renderer before assembly, because such a distribution builds successfully and is still unusable, and the Mini App source set must therefore not inherit Compose UI from a common source set.

Stage F's implementation is in place: the plugin registers a `miniapp` extension whose only host is `wechat`, carrying one build setting — the host bundle directory — with the host modelled as a `MiniAppHostConfiguration` subtype rather than as the platform. Business and backend configuration stays out of Gradle by design.
