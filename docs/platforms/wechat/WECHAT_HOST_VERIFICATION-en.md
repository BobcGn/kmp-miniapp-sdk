...
`NotRequested` cannot be produced on the account used for the runs below, because it has already recorded a decision for the mapped permission and WeChat then reports only `Granted` or `Denied`. Reproducing it would need a different account, a different device, or clearing the mini program's authorization history, and none of those were performed. The state is covered by the `HostVersion`-style unit and contract checks instead: the Fake Host contract checks, the adapter tests for a missing entry and for `true`/`false`/missing conversion, and the first-prompt success and refusal paths all run without a host. A page that has not queried yet shows `UNKNOWN`, which is an example-page placeholder and deliberately not a member of the public `PermissionState`.
