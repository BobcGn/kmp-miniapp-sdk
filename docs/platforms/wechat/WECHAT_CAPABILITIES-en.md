...
- `Resolved at runtime` — the SDK gates the capability and asks the host through `wx.canIUse` rather than comparing against a recorded figure. The official entry pages for the current Storage APIs, `wx.request`, `wx.getLocation`, `wx.scanCode`, and `wx.chooseMedia` do not state an introduction version for the API itself, so the repository does not infer one from a development machine. This is a live answer for the host actually running, not a missing value.
