...
BLE 是本 SDK 第一个建立在真实 `on`/`off` 事件源之上的能力，因此其测试针对的是资源模型而不是 Bluetooth：**24** 个 adapter 测试、**12** 个 interop 测试、**7** 个 session 测试，外加一个证明三个能力键彼此独立门控的 gate 检查。

它们断言的内容各自都有变异探针支撑：discovery 与 connection 调用到达宿主；SDK 无法读取的条目会被跳过并计数，而「不是设备数组」的载荷会以 `InvalidResponse` 结束流；空白设备标识在调用宿主前被拒绝；被取消的 collector 会移除**它注册的那个值**；正常结束或失败的 collector 同样移除；宿主拒绝的移除只被计数且不覆盖结果；两个 collector 不共享状态；重复设备只发一次事件；迟到的宿主回调不改变任何东西；取消不会被报告为宿主失败；停止后 session 的 listener 计数回到 0。

已执行并还原四个变异以证明测试确实会失败：删除 `awaitClose` 的移除（6 个测试失败）、用不同的 listener 值移除（2 个）、把取消吞成失败（1 个）、删除去重策略（1 个）。

**这些测试不需要 WeChat runtime，也不构成宿主证据。** 它们使用一个保留全部「已注册 / 已移除」listener 的 fake host，因此身份配对是被断言出来的，而不是被假定成立的。
...
7. 验证 Bluetooth 事件模型。它是 `Experimental`，其验收被拆开，因为宿主本身就是拆开的：在 macOS 上，已安装的
开发者工具基础库会回答 `createBLEConnection:fail API_NOT_SUPPORT`，因此模拟器可以验证 adapter 与 discovery，
但永远无法验证连接。

| 步骤 | 点击 | 预期 |
| --- | --- | --- |
| 1 | `Check BLE capability` | Console：`[kmp-miniapp-sdk] ble capability: PASS wechat.bluetooth-adapter=…, wechat.bluetooth-discovery=…, wechat.bluetooth-connection=…`。macOS 上连接键显示 gate 的查询结果，另外两项为 `Supported` |
| 2 | `Open adapter`、再 `Start discovery` | Console：`[kmp-miniapp-sdk] ble adapter: PASS opened=true`，随后 `ble discovery: STARTED`。设备被上报时逐个出现在卡片中 |
| 3 | `Refresh device count` | Console：`ble device: EVENT count=N duplicatePolicy=deviceId-dedup` 与 `ble cleanup: PASS listeners=N`。同一个设备被上报两次仍只计一次 |
| 4 | `Stop discovery`、再 `Close adapter` | Console：`ble discovery: STOPPED`，随后 `ble adapter: PASS opened=false` 与 `ble cleanup: PASS listeners=0` |

第 4 步的 `listeners=0` 正是这项能力存在的意义：它是 SDK 自己统计的宿主 listener 数；关闭之后仍大于 0 就意味着
消费者仍在为一个注册付出代价。Console 中任何位置都不出现设备标识、MAC 地址、广播数据或宿主原文：卡片只做脱敏
并只报告封闭分类。

连接需要真机与可达的外设；其步骤见 [WECHAT_HOST_VERIFICATION-ch.md](platforms/wechat/WECHAT_HOST_VERIFICATION-ch.md) 的连接相关行。

8. 核对版本门控的状态。Runtime Detection 卡片的期望状态来自宿主，因此不需要改动代码即可观察：
...
9. 验证权限生命周期。页面加载期间不会请求任何权限，因此以下每一步都由点击触发。卡片初始显示宿主已有的状态：全新安装或清除小程序授权数据后为 `NotRequested`。
...
10. 验证隐私授权流程。它与上面的权限是不同条件，二者分别记录。调试基础库必须为 2.32.3 或更高，且小程序必须在 MP 后台隐私指引中声明收集类型，否则宿主没有可授权的内容。
...
11. 验证微信会话检查。它与上面的 login bootstrap 分别记录，因为有效会话不是身份，只有 login code 路径才能建立身份。该检查刻意在页面加载时、bootstrap 之前执行：获取 code 会刷新客户端登录态，从而掩盖已过期的会话。
...
12. 验证剪贴板与震动。页面加载期间不会触碰剪贴板或震动器，因此以下每一步都由点击触发。剪贴板步骤只与页面自己写入的固定测试字符串比较；页面从不显示或记录剪贴板里可能存在的其他内容，那属于用户。
...
13. 验证文件系统。页面加载期间不会触碰文件系统，因此以下每一步都由点击触发。页面只向固定文件名写入固定的非敏感字符串，从不显示或记录沙箱根或其他文件的内容。
...
14. 验证定位。它由三个彼此独立的条件把关，因此以下步骤依次走过能力判定、隐私协议与权限。页面加载期间不会读取位置，因此每一步都由点击触发。
...
15. 验证扫码。它打开的是微信自身的扫码界面，因此页面加载期间不会打开任何界面，以下每一步都由点击触发。页面只报告内容是否存在、宿主报出的格式是否被识别，以及结果是成功、原因不可判定的中断还是其他失败；**从不显示或记录扫码内容、`rawData`、字符集或图片路径**。
...
16. 验证媒体选择。它打开的是微信自身的选择界面，因此页面加载期间不会打开任何界面，以下每一步都由点击触发。页面只报告返回了几个文件、宿主报出的每个类别是否被 SDK 识别、元数据是否可用；**从不显示或记录所选媒体、base64、文件名或完整临时路径**。
...
17. 验证订阅消息请求。微信要求用户手势，因此页面加载期间不会询问宿主，以下每一步都由点击触发。页面只打印计数，从不打印模板 ID：用户订阅了哪些模板，是用户与小程序之间的事，截图不得携带它。
...
18. 验证网络扩展。页面加载期间不会查询、注册监听、上传或下载，因此每一步都由点击触发。页面从不打印 URL、header、文件路径或响应正文；它报告状态、字节数、事件数与是否触发了 abort。
...
19. 验证标准支付。页面加载期间不会触碰支付界面，因此每一步都由点击触发。页面的日志行中不会出现任何支付参数、签名、商户标识或宿主自身的失败文本；失败会被归约为一个封闭标签。
...
20. 记录你观察到了哪些检查项、哪些没有观察到。只有在某项 capability 完成真实宿主运行后，才会在 [PROJECT_FACTS-ch.md](PROJECT_FACTS-ch.md) 中被记录为已通过宿主验证。
