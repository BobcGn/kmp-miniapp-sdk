...
Navigation needs interaction rather than a single page load. Tapping through the second and third pages prints `[kmp-miniapp-sdk] navigation: PASS <action>` for `wx.navigateTo`, `wx.redirectTo`, and `wx.navigateBack`; the tap sequence is in the checklist. The Tab Switching card switches to the tabtarget tab with `wx.switchTab` and prints `[kmp-miniapp-sdk] switch tab: PASS`; its second button asks for the third page, which has no tab, and prints only the closed category the SDK reported, never the host's own failure text. The Bluetooth card is experimental and exists to show the SDK's event-driven resource model rather than a Bluetooth API: it checks the three BLE capability keys, opens the adapter, starts and stops discovery, connects and disconnects, and prints `[kmp-miniapp-sdk] ble cleanup: PASS listeners=0` once every stream has ended. Device identifiers are masked, and a failure prints only a closed category. On macOS the simulator's base library refuses every connection call, so connecting needs a real device.
